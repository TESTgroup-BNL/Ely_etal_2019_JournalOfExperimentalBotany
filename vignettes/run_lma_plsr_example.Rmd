---
title: "Example application of Ely et al 2019 LMA PLSR model to existing data in EcoSIS"
author: "Shawn Serbin"
date: '2019-11-04'
output:
...
git_repo <- "https://raw.githubusercontent.com/TESTgroup-BNL/Ely_etal_2019_JournalOfExperimentalBotany/master/"
print("**** Downloading PLSR coefficients ****")
githubURL <- paste0(git_repo,"PLSR_model_coefficients/LMA/LMA_g_m2_PLSR_Coefficients_12comp.csv")
LeafLMA.plsr.coeffs <- source_GitHubData(githubURL)
rm(githubURL)
githubURL <- paste0(git_repo,"PLSR_model_coefficients/LMA/LMA_g_m2_Jackkife_PLSR_Coefficients.csv")
...
png(file=file.path("~",output_dir,'Angers_Lopex_spectra_summary_plot.png'),height=3000,
    width=3900, res=340)
...
temp <- as.matrix(sub_spec) %*% LeafLMA.plsr.coeffs.vec
leafLMA <- data.frame(rowSums(temp))+LeafLMA.plsr.intercept[,2]
leafLMA <- leafLMA[,1]  # convert to standard LMA units from sqrt(LMA)
...
# output results
write.csv(x = LeafLMA.PLSR.dataset.out, file = file.path("~",output_dir,"Angers_Lopex_PLSR_estimated_LMA_data.csv"),
          row.names = F)
...
png(file=file.path("~",output_dir,'Angers_Lopex_LMA_validation_plot.png'),height=3000,
    width=3900, res=340)
